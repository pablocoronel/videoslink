				...
				<div id='MP_fondo'>
					...
					<div id='MP_listado'>
						<ul id='barra_listado'>
							...
							<li id='env_tit'><span class='listado_titulo'>Para</span></li>
					...
					$sql_pag="SELECT mensajes_privados.*, registro_usuarios.usuario FROM mensajes_privados 
								INNER JOIN registro_usuarios ON mensajes_privados.id_destinatario = registro_usuarios.id 
								WHERE id_remitente = '$_GET[user]' AND borrado_remitente = 'no'";
					...
					if($total_mp > 0){
						$sql="SELECT mensajes_privados.*, registro_usuarios.usuario FROM mensajes_privados INNER JOIN registro_usuarios
								ON mensajes_privados.id_destinatario = registro_usuarios.id WHERE id_remitente = '$_GET[user]' AND borrado_remitente = 'no'
					...
					?>
						<ul id='barra_listado'>
							...
							<li id='tit'><a href="MP_ver_salida.php?user=<?php echo $_SESSION['login_id']; ?>&id_MP=<?php echo $fila['id']; ?>"><span class='listado_titulo'><?php echo $fila['titulo']; ?></span></a></li>
							<li id='env'><a href="../perfil.php?id_user=<?php echo $fila['id_destinatario']; ?>"><span class='listado_titulo'><?php echo $fila['usuario']; ?></span></a></li>
							<li id='fec'><span class='listado_titulo'><?php echo $fila['fecha_envio']; ?></span></li>
						</ul>
					<?php
						}
					}else{
					...
					<!-- PAGINADO -->
					<div id='paginado'>
						<a href="MP_salida.php?user=<?php echo $_SESSION['login_id']; ?>&pag=<?php echo $pag_anterior; ?>">
							Pagina Anterior
						</a>
						<a href="MP_salida.php?user=<?php echo $_SESSION['login_id']; ?>&pag=<?php echo $pag_siguiente; ?>">
							Pagina Siguiente
						</a>
					</div>
					</div>
