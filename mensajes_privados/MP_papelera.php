				...
				<!-- BANDEJAS -->
					...
					<div id='MP_listado'>
						<ul id='barra_listado'>
							...
							<li id='fec_tit'><span class='listado_titulo'>Para</span></li>
						</ul>
					<?php
					/* CUENTAS PARA PAGINADO */
					$sql_pag="SELECT mensajes_privados.*, registro_usuarios.usuario FROM mensajes_privados INNER JOIN registro_usuarios
								ON mensajes_privados.id_remitente = registro_usuarios.id
								WHERE (mensajes_privados.id_destinatario = '$_GET[user]' AND mensajes_privados.borrado_destinatario = 'si')
								OR (mensajes_privados.id_remitente = '$_GET[user]' AND mensajes_privados.borrado_remitente = 'si')";
					...
					if($total_mp > 0){
						...
						while($fila= mysqli_fetch_assoc($res)){
							$sql_2="SELECT usuario FROM registro_usuarios WHERE id = '$fila[id_destinatario]'";
							$res_2= mysqli_query($conexion,$sql_2);
							while($fila_2= mysqli_fetch_assoc($res_2)){
					?>
						<ul id='barra_listado'>
							<li id='che'><span class='listado_titulo'><input type='checkbox' name='MP'></span></li>
							<li id='tit'><a href="MP_ver_papelera.php?user=<?php echo $_SESSION['login_id']; ?>&id_MP=<?php echo $fila['id']; ?>"><span class='listado_titulo'><?php echo $fila['titulo']; ?></span></a></li>
							<li id='env'><a href="../perfil.php?id_user=<?php echo $fila['id_remitente']; ?>"><span class='listado_titulo'><?php echo $fila['usuario']; ?></span></a></li>
							<li id='fec'><a href="../perfil.php?id_user=<?php echo $fila_2['id_destinatario']; ?>"><span class='listado_titulo'><?php echo $fila_2['usuario']; ?></span></a></li>
						</ul>
					<?php
							}
						}
					...
					<!-- PAGINADO -->
					<div id='paginado'>
						<a href="MP_papelera.php?user=<?php echo $_SESSION['login_id']; ?>&pag=<?php echo $pag_anterior; ?>">
							Pagina Anterior
						</a>
						<a href="MP_papelera.php?user=<?php echo $_SESSION['login_id']; ?>&pag=<?php echo $pag_siguiente; ?>">
							Pagina Siguiente
						</a>
					</div>
					</div>
